...
class Disassembler(object):
	def __init__(self, binary):
		...
		#binary += b'\x00'
		binary += "00"
		#binary = str(binary) + '\x00'
		#binary = binary.encode('utf-8') + b'\x00'
		...
		for i in range(0, len(binary), 2):
			try:
				...
				#print(f"illegal hex character at index {i}: {binary[i:i + 2]}")
	...
	def __detect_swarm_hash(self, binary):
		...
		# 0xa1 0x65 'b' 'z' 'z' 'r' '0'
		swarm_pattern = re.compile(b"a165627a7a7230")
		match = re.search(swarm_pattern, binary.encode())
		if not match:
    ...
    # 0xa1 0x65 'b' 'z' 'z' 'r' '0'
		...
		if not match:
			...
			self.swarm_hash_address = match.start() // 2
"""
